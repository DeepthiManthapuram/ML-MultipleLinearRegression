#same as simple linear regression but tiny changes in data preview,m & c, predictions
...
st.markdown(f"""
    ...
    <b>co-efficient (Group size):</b> {model.coef_[1]:.1f}<br>
...
bill_amount = st.slider("Total Bill ($)", float(df["total_bill"].min()), float(df["total_bill"].max()), 50.0)

size = st.slider("Group Size", int(df["size"].min()), int(df["size"].max()), 2)

input_scaled = scaler.transform([[bill_amount,size]])
